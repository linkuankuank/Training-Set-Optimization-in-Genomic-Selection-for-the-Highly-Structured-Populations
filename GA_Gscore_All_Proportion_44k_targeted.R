#### OPT-train ####
opt.train.gscore.all.proportion.targeted  = function(X, n , test, th = 10^-4, lambda = NULL,CD_Criteria){
  ## Initial
  N = nrow(X); n0 = length(test); Nc=N-n0
  cand = seq(N)[-test]
  ...
  sol = c()
  for (i in 1:ss) {
    a = sort(STRATE_367(c(1:N),test,n))  #分層抽樣
    b = matrix(0,N,1)
    b[a,1] = 1
    b=b[-test,]
    sol = cbind(sol,b)
...
opt.train.gscore.all.proportion.targeted   = cmpfun(opt.train.gscore.all.proportion.targeted) 
